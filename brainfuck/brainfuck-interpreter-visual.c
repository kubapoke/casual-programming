...
void displayState(char* ip, char* cp, char* instructions, char* cells, char* fp)
{
    for(int i = 0; i < strlen(instructions); i++)
        putchar(instructions[i]);
    putchar('\n');
    for(char* i = ip; i > instructions; i--)
        putchar('-');
    putchar('^');
    putchar('\n');
    
    for(char* i = cells; i < fp; i++)
        putchar(*i);
    putchar('\n');
    for(char* i = cp; i > cells; i--)
        putchar('-');
    putchar('^');
    putchar('\n');
}
...
void interpret(char* instructions, int* brackets)
{
    ...
    printf("%c\n",*cp);
        if(cp > fp)
            fp = cp;
        displayState(ip, cp, instructions, cells, fp);
